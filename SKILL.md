...
2. **選引擎**：
   ...
   | 引擎 | 說明 | 實測（M4，25.9 分鐘會議）|
   |------|------|------|
   | `mlx`（預設）| Apple Silicon GPU | **2 分 48 秒**（9.3x realtime），峰值記憶體 980MB |
   | `cpu` | 原本的 openai-whisper／PyTorch CPU，保留當 fallback | 約 35 分鐘，CPU 工作量多 40 倍、會降頻 |
   | `both` | 兩個都跑，輸出到 `<輸出資料夾>/mlx` 與 `/cpu`，印並排比較表 | mlx 時間 + cpu 時間 |

   - 非 Apple Silicon、或 MLX 準備失敗 → **自動退回 `cpu`**，不會中斷。
...
4. **自動轉繁**（預設開啟，不用做任何事）：Whisper 兩個引擎都會漏簡體字（實測 0.5–3%，跟段落內容有關），轉錄完會自動用 OpenCC **`s2tw`**（台灣標準字形）就地統一，五種格式都處理。腳本會印「修正 N 個簡體字」。
   ...
   - opencc 首次會自動裝（優先裝進 MLX venv）。裝不起來會印警告但**不中斷轉錄**。
...
- **本 skill 不做 speaker diarization**。混音 mono 的講者歸屬只能靠內容判讀。若某次真的需要機器分軌，那要走雲端（如 OpenAI `gpt-4o-transcribe-diarize`，約 $0.006/分鐘，但**音檔要上傳**，與本 skill 的不外傳前提衝突——需先向 Ray 確認該檔案可否上傳）。
