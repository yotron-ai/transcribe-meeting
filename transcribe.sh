...
if ! WHISPER="$(find_whisper)"; then
  echo "未偵測到 openai-whisper，正在安裝（首次約需幾分鐘）..."
  python3 -m pip install --user -U openai-whisper >/dev/null
  WHISPER="$(find_whisper)" || { echo "安裝後仍找不到 whisper，請確認 pip 安裝路徑"; exit 1; }
fi
...
if [ -n "${PROMPT}" ]; then
  "${WHISPER}" "${INPUT}" \
    --language "${WHISPER_LANG}" --model turbo --output_format all \
...
echo "提醒：近靜音段 Whisper 可能生幻覺(如「尼泊爾」「字幕by…」)，標註前人工掃一遍刪掉。"
